...
// Check if $_GET['model_name'] is set before accessing it
...
<head>
  ...
  <style>
        body {
            ...
            background-color: transparent; /* Initially set the background color to transparent */
        }
        ...
        .loader {
            ...
            bottom: 0;
            right: 0;
            z-index: 999; /* Ensure the loader appears above other content */
            display: block; /* Initially display the loader */
        }
...
<body>
<div class="blurry-background" style="background-color: transparent; width: 100%; height: 100%; padding: 10px; " >
  <a href="home.php" style="color:red;background-color: transparent;">
						<i class="fa-duotone fas fa-home"></i>
					</a>

					<a href="buttons.php" style="margin-left: 5px; color: blue;background-color: transparent;">
						<i class="fa-duotone fa fa-arrow-left"></i>
					</a></div>
<div id="positionDisplay"> Camera Position: X=0.00, Y=0.00, Z=0.00, Rotation: X=0.00, Y=0.00, Z=0.00
  
</div>
<div id="controls">
    <h3>Controls</h3>
    <p>Use <span style="color: blue;"> W, A, S, D</span> keys to move camera</p>
    <p>Use <span style="color: blue;">Q, E</span> keys to rotate camera</p>
    <p>Use <span style="color: blue;">Z, X </span>keys to rotate camera along X-axis</p>
    <p>Use <span style="color: blue;">Space, Shift</span> keys to move camera up and down</p>
    <p>Use <span style="color: blue;">Backspace</span> to reset camera rotation</p>
  </div>
  ...
  <script type="module">
    // Function to hide the loader and set background color back to transparent
    function hideLoader() {
        document.getElementById('loader').style.display = 'none';
        document.body.style.backgroundColor = 'transparent';
    }

    // Show loader initially and hide it after 5 seconds
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // Hide loader and set background color back to transparent after 5 seconds
        setTimeout(hideLoader, 5000); // 5000 milliseconds = 5 seconds
    });
...
const productMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000 });
const productMaterial2 = new THREE.MeshBasicMaterial({ color: 0xffff00 });
...
// Function to update the camera position display
function updatePositionDisplay() {
  const positionDisplay = document.getElementById('positionDisplay');
  positionDisplay.textContent = `Camera Position: X=${camera.position.x.toFixed(2)}, Y=${camera.position.y.toFixed(2)}, Z=${camera.position.z.toFixed(2)}, Rotation: X=${camera.rotation.x.toFixed(2)}, Y=${camera.rotation.y.toFixed(2)}, Z=${camera.rotation.z.toFixed(2)}`;
}
...
function animate() {
  ...
  updatePositionDisplay(); // Call the function to update the position display
}
    animate();
